...
# project changes that README and no page.
#
# Named, though it has only one use site today, where `_REPO_URL` above is named because it
# has two and they could drift. The reason here is the clause rather than the drift: it is
# *exactly one* link, and a constant is what makes a second one an edit somebody has to mean.
