...
The model beats the persistence baseline by 14.8% on MAE while scoring R² 0.021.
...
- The R² prose says "the average of the period being scored", not "the long-run average",
  and states plainly that a better model reaches 0.23 on the same window.
...
1. **Resolve the model choice, out loud.** `compare_models` builds three candidates and
   `pipeline.compare` prints the winner to stdout; nothing persists it and
   `train_forecaster` hardcodes RandomForest. Either close the loop or state the RF choice
   as a deliberate interpretability trade-off with the cost named (−0.51 MAE, −0.21 R²).
   "You have a model comparison — how does it reach production?" currently answers "a human
   reads stdout."

2. **Encode wind direction as u/v components.** `features.py` passes every weather column
...
3. **Regime breakdown at the WHO 15 µg/m³ line**, plus mean error (bias). The original draft
...
4. **Backtest chart, last ~14 days.** Trap to avoid: the saved bundle holds the *all-data*
...
5. **Feature importances,** with the corrected caption from the finding above. Note the
   coupling: `HistGradientBoostingRegressor` has no `feature_importances_`, so item 1 decides
   whether this stays a one-liner or becomes `permutation_importance`.

6. **Multi-horizon with lead time as a feature.** Every serving row uses lags ≥24h from the
...
7. **Prediction intervals.** sklearn ≥1.5 is pinned, so
   `HistGradientBoostingRegressor(loss="quantile")` is two extra fits — but on RF it needs
   per-tree spread or a third-party quantile forest. Sequence after item 1.

### Later

8. **Prospective forecast log,** not "metrics over time". The original framing was both
   ...
   observations arrive. That is genuine prospective evaluation and it makes item 4 real
   rather than retrospective. Cost M, not S.

9. **Health context** — translate µg/m³ into what it means for a runner or someone with
   asthma. Low technical signal, real product signal.

10. **Polish version of the page.** Data and likely audience are Polish; code and docs stay
    English by project convention.

11. **Extract a pure `_render_page(...)` from `generate_report`.** Page composition is
...
12. **Three-station comparison is not available as described.** `docs/research/data-sources.md`
...
README and the published page, capacity is not the constraint. Revisit once items 1–4 land.
